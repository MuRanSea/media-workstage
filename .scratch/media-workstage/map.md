...
- [01-canvas-engine-selection](issues/01-canvas-engine-selection.md): 选定空间无限画布引擎架构（Spatial Canvas），实现紧凑美观卡片（~380px）与 Tab 抽屉折叠、零漂移光标定点缩放、多图参考胶囊与 @Prompt 绑定。原型分支 `prototype/canvas-engine`。
